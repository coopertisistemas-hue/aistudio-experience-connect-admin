...
export default function DashboardTransfers() {
  ...
  const upcoming = mockTransfers.filter((t) => t.status !== 'completed' && t.status !== 'cancelled').slice(0, 5);
              ...
              return (
                <tr key={t.id} className="group hover:bg-sand-50 transition-colors duration-100">
                  ...
                  <td className="py-3 pr-4 min-w-[160px]">
                    <p className="text-xs font-medium text-navy-800 leading-snug">{t.origin}</p>
                    <div className="flex items-center gap-1 mt-0.5">
                      <i className="ri-arrow-right-line text-navy-300 text-[10px]"></i>
                      <p className="text-[11px] text-navy-400 leading-snug">{t.destination}</p>
                    </div>
                  </td>
                  <td className="py-3 pr-4">
                    {t.driver_name ? (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-navy-700 whitespace-nowrap">{t.driver_name.split(' ')[0]}</span>
                      </div>
                    ) : (
                  ...
                  <td className="py-3 pr-4">
                    <span className="text-xs text-navy-600 whitespace-nowrap">{t.vehicle_name}</span>
                    <p className="text-[10px] text-navy-300 font-mono">{t.vehicle_plate}</p>
                  </td>
                  <td className="py-3 pr-4">
