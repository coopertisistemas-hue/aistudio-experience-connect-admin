...
interface Props {
  ...
  onChange: (f: ReceivablesFilters) => void;
  total: number;
  filtered: number;
}

const STATUS_PILLS: { value: ReceivableStatus | 'all'; label: string }[] = [
...
export default function ReceivablesFilterBar({ filters, onChange, total, filtered }: Props) {
  ...
  const clear = () => onChange({ search: '', status: 'all', method: 'all', period: 'all' });
    ...
    <div className="mb-5 space-y-3">
      <div className="flex items-center gap-3">
        <div className="flex-1 relative">
          ...
          <input
            type="text"
            placeholder="Buscar por reserva, passageiro, rota..."
      ...
      {/* Advanced */}
      {expanded && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-4 bg-stone-50 border border-stone-200 rounded-xl">
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-semibold text-stone-500 uppercase tracking-wide">Período</label>
            <select
              value={filters.period}
              onChange={(e) => onChange({ ...filters, period: e.target.value as ReceivablesFilters['period'] })}
              className="w-full h-8 px-2.5 text-sm bg-white border border-stone-200 rounded-lg text-stone-700 focus:outline-none focus:ring-2 focus:ring-teal-400/40 cursor-pointer"
            >
      ...
      {(filters.search || activeCount > 0) && (
        <p className="text-[12px] text-stone-500">
          Exibindo <span className="font-semibold text-stone-700">{filtered}</span> de {total} recebíveis
        </p>
      )}
    </div>
  );
