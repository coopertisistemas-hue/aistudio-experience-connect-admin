...
import { mockNotifications, notificationStats } from '@/mocks/admin-notifications';
...
export default function NotificationsPage() {
    ...
    return notifications.filter((n) => {
      if (filters.search) {
        ...
        if (
          ...
          !(n.entity_ref?.toLowerCase().includes(q) ?? false)
        ) return false;
      }
  ...
  const liveStats = useMemo(() => ({
    ...
    opConflicts:   notifications.filter((n) =>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h1 className="font-serif text-2xl font-semibold text-navy-950 leading-tight">
                  Central de Notificações
                </h1>
                <p className="text-navy-400 text-xs font-light mt-0.5">
                  Alertas operacionais, pagamentos, transfers e disponibilidade
                </p>
              </div>
            </div>
          </div>
