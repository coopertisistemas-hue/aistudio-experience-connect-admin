...
import { mockDailyStats, mockHourlyPeaks, mockRouteAnalytics } from '@/mocks/admin-reports';
...
export default function ReportsTransferVolume() {
  ...
  // By category
  const byCat = mockRouteAnalytics.reduce<Record<string, { transfers: number; revenue: number }>>((acc, r) => {
      ...
      <div className="p-5 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <div className="overflow-x-auto">
              <svg width={mockHourlyPeaks.length * 22} height={80} className="block">
                {(() => {
                  const maxV = Math.max(...mockHourlyPeaks.map((h) => h.transfers), 1);
                  return mockHourlyPeaks.map((h, i) => {
                ...
                <line x1={0} y1={60} x2={mockHourlyPeaks.length * 22} y2={60} stroke="#e7e5e4" strokeWidth={1} />
              </svg>
            </div>
