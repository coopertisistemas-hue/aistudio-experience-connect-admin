...
export default async (req: Request): Promise<Response> => {
  ...
  const { tenant_id, booking_id, payment_id } = body;
  if (!tenant_id || !booking_id || !payment_id) {
  ...
  try {
    ...
    const idempotencyKey = `confirm-${booking_id}-${payment_id}-${Date.now()}`;

    const { data, error } = await adminClient.rpc('confirm_booking_from_payment', {
    ...
    return new Response(JSON.stringify({ success: true, confirmed: data }), {
      status: 200,
